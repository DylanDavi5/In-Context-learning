...
def get_model_from_run(run_path, step=-1, only_conf=False, name=None):
    ...
    model = models.build_model(conf.model, name)
...
def eval_batch(model, task_sampler, xs, xs_p=None):
    ...
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "lstm"]:
        ...
        device = "cpu"
    if model.name=="combination":
        if xs_p is None:
            splitys=task.get_split_eval(xs)
            ys=task.evaluate(xs)
            for i in range(len(splitys)):
                splitys[i]=splitys[i].to(device)
            pred = model(xs.to(device), splitys, inds=[i]).detach()
            ...
            for i in range(n_points):
                ...
                splitys=task.get_split_eval(xs_comb)
                ys=task.evaluate(xs_comb)
                for i in range(len(splitys)):
                    splitys[i]=splitys[i].to(device)
                pred = model(xs_comb.to(device), splitys, inds=[i]).detach()
...
def build_evals(conf):
    ...
    if "task_kwargs" in conf["training"]:
...
def compute_evals(all_models, evaluation_kwargs, save_path=None, recompute=False, names=None, num_eval_examples=1280, batch_size=64):
    try:
    ...
    for eval_name, kwargs in tqdm(evaluation_kwargs.items()):
        ...
        for i, model in enumerate(all_models):
            if names==None:
                ...
                metrics[model.name] = eval_model(model, **kwargs, num_eval_examples=num_eval_examples)
            else:
                if names[i] in metrics and not recompute:
                    continue
                metrics[names[i]] = eval_model(model, **kwargs, num_eval_examples=num_eval_examples)
